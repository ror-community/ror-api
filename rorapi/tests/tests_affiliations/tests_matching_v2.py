...
class AffiliationMatchingSpringerSingleSearchTestCase(SimpleTestCase):
...
class AffiliationMatchingCrossrefSingleSearchTestCase(SimpleTestCase):
    def match(self, affiliation):
        affiliation = re.sub(r'([\+\-=\&\|><!\(\)\{\}\[\]\^"\~\*\?:\\\/])',
                             lambda m: '\\' + m.group(), affiliation)
        results = requests.get('{}/v2/organizations'.format(API_URL), {
