...
def get_ror_id(string):
    ...
    m = re.match(
        r"^(?:(?:(?:http|https):\/\/)?ror\.org\/)?(0\w{6}\d{2})$", unquote(string)
    )
    if m is not None:
...
def check_status_adv_q(adv_q_string):
    ...
    status_fields = [f for f in adv_query_fields if f.endswith("status")]
...
def validate(params, version):
    ...
    illegal_names = [k for k in params.keys() if k not in ALLOWED_PARAM_KEYS]
    errors = ["query parameter '{}' is illegal".format(n) for n in illegal_names]
...
def build_search_query(params, version):
    ...
    if "filter" in params or (not "all_status" in params):
        ...
        if (
            (not "status" in filter_dict)
            and (not "all_status" in params)
            and ror_id is None
        ):
